...
export type UnitKind = 'agent' | 'civilian' | 'guard' | 'enemy' | 'target';
 ...
 * civilians come freely, armed minds take a crowd behind you. The mission
 * target can never be persuaded.
 */
...
export function createUnit(
  ...
  return {
    id,
    kind,
    faction: kind === 'agent' ? 'player' : kind === 'civilian' ? 'neutral' : 'hostile',
    x: (tile % mapW) + 0.5,
