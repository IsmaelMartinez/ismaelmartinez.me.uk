...
Arcade pages live under `src/pages/[lang]/fun/`. Newer games (Tank Duel) split pure game logic into DOM-free TypeScript modules under `src/games/<game>/` (unit-tested in `tests/games/`) with a single `init<Game>()` entry the page calls; shared utilities (fixed-timestep loop, localStorage scores) live in `src/games/engine/`. Static labels render server-side via `useTranslations`; runtime-composed strings pass through `data-t-*` attributes on the game root. New games register a cabinet in `src/pages/[lang]/fun/index.astro`. Designs for the next games (Lemmings, Theme Park, SimCity) are in `docs/plans/2026-06-10-arcade-expansion-design.md`.
