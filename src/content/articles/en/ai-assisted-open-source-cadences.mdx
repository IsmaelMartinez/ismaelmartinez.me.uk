...
[bonnie-wee-plot](https://github.com/IsmaelMartinez/bonnie-wee-plot) is seasonal, a Scottish allotment app where the features that matter in March (sowing windows) are different from the ones that matter in August (harvest, blight watch). The codebase grows in chapters, with the gardening year telling it what to build next.

[votescot](https://github.com/IsmaelMartinez/votescot) is ephemeral, an open-source vote compass built for the 2026 Scottish Parliament election. The relevance window is narrow; the code has to be solid for the duration, but the lifecycle has a built-in end.

[wifisentinel](https://github.com/IsmaelMartinez/wifisentinel) is reactive, built quickly because a specific situation needed it (a dodgy Amsterdam Airbnb where I wanted to know what was actually on the network). What started as a one-off grew into a real toolkit once the original problem was solved.
