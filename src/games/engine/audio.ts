...
export function createGameAudio(options: GameAudioOptions): GameAudio {
  ...
  function playSfx(name: SfxName): void {
    ...
    switch (name) {
      case 'blip':
        playTone(hz[0], now, 0.08, 'square', 0.5, out);
        break;
      case 'score':
        playTone(hz[0], now, 0.09, 'square', 0.5, out);
        playTone(hz[1], now + 0.08, 0.1, 'square', 0.5, out);
        break;
      case 'hit':
        playTone(hz[0], now, 0.14, 'sawtooth', 0.6, out);
        playTone(hz[1], now + 0.04, 0.16, 'sawtooth', 0.5, out);
        break;
      case 'explosion': {
      ...
      case 'gameover':
        playTone(hz[0], now, 0.18, 'triangle', 0.5, out);
        playTone(hz[1], now + 0.16, 0.18, 'triangle', 0.5, out);
        playTone(hz[2], now + 0.32, 0.3, 'triangle', 0.5, out);
        break;
      case 'rescue': {
        ...
        for (let i = 0; i < hz.length; i++) {
          ...
          playTone(hz[i], now + i * 0.06, last ? 0.2 : 0.1, 'triangle', 0.5, out);
        }
        break;
      }
    }
